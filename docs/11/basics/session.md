...
使用 `database` 会话驱动时，你需要确保你有一个数据库表用于包含会话数据。通常，这包括在 Laravel 的默认 `0001_01_01_000000_create_users_table.php` [数据库迁移](/docs/11/migrations) 中；但是，如果由于某种原因你没有 `sessions` 表，你可以使用 `make:session-table` Artisan 命令来生成此迁移：
